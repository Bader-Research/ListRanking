...
int list_ranking(int n, int k, list_t *List, THREADED)
{
  ...
  register int i, j, s, target, current, prev;
  register LISTRANK_TYPE val;
  int block, start, finish, group, group_log, group_m1, times,
  ...
  Sums = (int *) node_malloc((THREADS)*sizeof(int), TH);
  Master = (master_t *) node_malloc((s+1)*sizeof(master_t),TH); 
  Succ = (int *) node_malloc(n*sizeof(int),TH);
